 ...
 */
async function callGeminiAPI(prompt, apiKey, platformContext) {
 ...
 */
async function callOpenAIAPI(prompt, apiKey, endpoint, model, platformContext) {
    return new Promise((resolve) => {
...
async function fetchWikipediaKnowledge(query) {
    return new Promise((resolve) => {
        ...
        const req = https_1.default.get(url, options, (res) => {
            let data = '';
            res.on('data', (chunk) => {
                data += chunk;
            });
            res.on('end', () => {
                try {
                    ...
                    const parsed = searchResults.slice(0, 3).map((r) => ({
...
const askDoubt = async (req, res) => {
    try {
        const { question, category = 'General', skillTitle } = req.body;
        ...
        try {
            ...
            if (searchTerms.length > 0) {
                ...
                dbMatchedSkills = await Skill_1.default.find({
                    $or: [
            ...
            const allSkills = await Skill_1.default.find().limit(6).exec();
        ...
        if (/\b(tell about me|tell me about me|who am i|my profile|my credits|my skills|my details|about me|show my profile|my account)\b/i.test(qTrimmed)) {
            ...
            let userWantedSkills = [];
            try {
                if (req.user?.id) {
                    const u = await User_1.default.findById(req.user.id);
                    if (u) {
                        userName = u.name || userName;
                        userEmail = u.email || '';
                        userCredits = u.credits ?? 100;
                    }
                    const userSkills = await Skill_1.default.find({ user: req.user.id });
...
4. **No Real Money**: Pure skill exchange powered by community collaboration.`;
        ...
        else if (dbMatchedSkills.length > 0 &&
